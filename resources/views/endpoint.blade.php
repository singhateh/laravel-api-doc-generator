...
@section('content')
    ...
    @php
        $requiredClasses = $param['required'] 
            ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' 
            : 'bg-gray-100 dark:bg-gray-900/30 text-gray-600 dark:text-gray-400';
        $requiredText = $param['required'] ? 'Required' : 'Optional';
    @endphp

    <div class="flex flex-col p-3 bg-gray-50 dark:bg-dark-700 rounded-lg mb-2">
        <div class="flex flex-wrap items-center gap-2">
            <span class="font-mono text-sm text-gray-900 dark:text-white">{{ $param['name'] }}</span>
            <span class="px-2 py-1 bg-gray-200 dark:bg-dark-600 text-gray-600 dark:text-gray-400 text-xs rounded">{{ $param['type'] }}</span>
            <span class="px-2 py-1 {{ $requiredClasses }} text-xs rounded">{{ $requiredText }}</span>
        </div>
...
@endforeach
                ...
                <!-- Success Response -->
           <!-- Success Response -->
@if(isset($endpoint['responses']['examples']))
<div class="space-y-6 bg-white dark:bg-gray-800">
    @foreach($endpoint['responses']['examples'] as $label => $example)
        @php
            $isSuccess = strtolower($label) === 'success';
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-800 shadow-sm">
            <h4 class="flex items-center gap-2 font-semibold {{ $titleColor }}">
                <i class="fa-solid {{ $icon }} {{ $iconColor }}"></i>
                {{ ucfirst($label) }} Response
            </h4>
            <pre class="bg-gray-900 dark:bg-gray-900 text-green-300 text-sm rounded-lg p-4 mt-2 overflow-x-auto">
<code class="language-json">{{ json_encode(json_decode($example), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</code>
            </pre>
        </div>
    @endforeach
</div>
